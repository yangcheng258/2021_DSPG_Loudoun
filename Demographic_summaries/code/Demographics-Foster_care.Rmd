...
foster_care_2020_virginia <- read_excel("~/Desktop/Virginia-Tech/DSPG-2021/Loudoun-County/2021_DSPG_Loudoun/Demographic_summaries/data/foster-care-2020.xlsx")

foster_care_2020 <- read_excel("~/Desktop/Virginia-Tech/DSPG-2021/Loudoun-County/2021_DSPG_Loudoun/Demographic_summaries/data/foster-care-2020-all.xlsx")
...
barplot(value$Value, xlab = "Age Groups", ylab = "Totals", main = "Age Groups in Foster Care (VA)", col = rainbow(length(value$Value)), names.arg = groups, cex.names = .6)
value$Age.Group<-factor(value$Age.Group, levels = value$Age.Group)

ggplot(data = value, aes(x= Age.Group, y = Value, fill = Age.Group) ) + 
    geom_col() + coord_flip() + theme(legend.position = "none")+
    labs(title = "Age Demographics of Foster Care",
         x = "",
         y = "Population Estimate", 
         caption = "Data Source: The Adoption and Foster Care Analysis and Reporting System FY 2019")

## pie chart using percentages 
pie(percent$Value, groups, main = "Age Groups in Percentages", col = rainbow(length(percent$Value)))
...
ggplot() + geom_col(mapping = aes(x = Value, y = Gender, fill = c("plum", "lightblue")), data = sex_df)+coord_flip()+ 
    theme(legend.position = "none") + 
    labs(title = "Sex Demographics of Foster Care", 
         x = "Population Estimate", y = "",
         caption = "Data Source: The Adoption and Foster Care Analysis and Reporting System FY 2019")

## pie chart for gender of Foster Care in Loudoun County 
pie(sex_df$Value, sex_df$Gender, main = "Gender of Foster Care Youth in Loudoun County", col = rainbow(length(sex_df$Value)) ) 
...
race <- data.frame(foster_care_2020[c(8,10,14,18),])
colnames(race) <- c("Race", "Value")
races <- c("Black", "White", "Asian", "Multi") 
...
ggplot(data = race, aes(x= races, y = Value, fill = Race) ) + 
    geom_col() + theme(legend.position = "none")+labs(title = "Racial Demographics of Foster Care", 
                                                      y = "Population Estimate", 
                                                      x = "", 
                                                      caption = "Data Source: The Adoption and Foster Care Analysis and Reporting System FY 2019")+scale_fill_brewer(palette="Dark2")


ggplot(data = data2, aes(x= age_19, y = value, fill = age_19) ) + 
    geom_col() + theme(legend.position = "none",
                       plot.background = element_rect(inherit.blank = T))+labs(title = "TAYs vs Children in Foster Care", 
                                                      y = "Population Estimate", 
                                                      x = "", 
                                                      caption = "Data Source: The Adoption and Foster Care Analysis and Reporting System FY 2019")+scale_fill_brewer(palette="Greens")+coord_flip()
...
pie(data2$value,
    ...
    main = "19+ Foster Care Youth in Loudoun County")
...
[1] Jordan,Elizabeth,. Fryar,Garet,. DeVooght, Kerry,. "Supporting Young People Transitioning from Foster Care: Virginia Findings from a National Survey and Policy Scan". Child Trends for the Better Housing Coalition and Children’s Home Society of Virginia. November 2017. https://www.childtrends.org/wp-content/uploads/2017/11/SYPTFC-Virginia-Findings-from-a-National-Survey-and-Policy-Scan-11.29.17.pdf. 
